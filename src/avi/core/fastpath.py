...
_COMMAND_TEMPLATES: tuple[tuple[re.Pattern[str], str], ...] = (
    (
        re.compile(
            r"^(what\s+command\s+shows?|how\s+do\s+i\s+show)\s+(the\s+)?current\s+directory$", re.I
        ),
        "pwd",
    ),
    (
        re.compile(
            r"^(what\s+command\s+(lists?|shows?)|how\s+do\s+i\s+(list|show))\s+(the\s+)?files(\s+here|\s+in\s+this\s+directory)?$",
            re.I,
        ),
        "ls -la",
    ),
    (
        re.compile(
            r"^(what\s+command\s+(checks?|shows?)|how\s+do\s+i\s+check)\s+(disk\s+space|disk\s+usage)$",
            re.I,
        ),
        "df -h",
    ),
    (
        re.compile(
            r"^(what\s+command\s+(shows?|lists?)|how\s+do\s+i\s+(show|list))\s+(running\s+)?processes$",
            re.I,
        ),
        "ps aux",
    ),
    (
        re.compile(
            r"^(what\s+command\s+shows?|how\s+do\s+i\s+show)\s+(my\s+)?current\s+git\s+branch$",
            re.I,
        ),
    ...
    (
        re.compile(
            r"^(what\s+command\s+(checks?|shows?)|how\s+do\s+i\s+check)\s+(git\s+status|repository\s+status)$",
            re.I,
        ),
        "git status",
    ),
    (
        re.compile(
            r"^(what\s+command\s+(shows?|lists?)|how\s+do\s+i\s+(show|list))\s+(recent\s+)?git\s+commits$",
            re.I,
        ),
        "git log --oneline -10",
    ),
    (
        re.compile(
            r"^(what\s+command\s+shows?|how\s+do\s+i\s+show)\s+(the\s+)?current\s+date$", re.I
        ),
        "date",
    ),
    (
        re.compile(
            r"^(what\s+command\s+shows?|how\s+do\s+i\s+show)\s+(the\s+)?current\s+user$", re.I
        ),
        "whoami",
    ),
    (
        re.compile(
            r"^(what\s+command\s+shows?|how\s+do\s+i\s+show)\s+(the\s+)?operating\s+system$", re.I
        ),
        "uname -a",
    ),
    (
        re.compile(
            r"^(what\s+command\s+(finds?|checks?)|how\s+do\s+i\s+(find|check))\s+(the\s+)?python\s+path$",
            re.I,
        ),
        "which python3",
    ),
    (
        re.compile(
            r"^(what\s+command\s+(checks?|shows?)|how\s+do\s+i\s+check)\s+(listening\s+)?(tcp\s+)?ports$",
            re.I,
        ),
        "ss -tulpn",
    ),
    (
        re.compile(
            r"^(what\s+command\s+(shows?|checks?)|how\s+do\s+i\s+(show|check))\s+(git\s+)?changes$",
            re.I,
        ),
        "git diff",
    ),
    (
        re.compile(
            r"^(what\s+command\s+shows?|how\s+do\s+i\s+show)\s+(the\s+)?(system\s+)?uptime$", re.I
        ),
        "uptime",
    ),
    (
        re.compile(
            r"^(what\s+command\s+(shows?|checks?)|how\s+do\s+i\s+(show|check))\s+(free\s+)?(memory|ram)(\s+usage)?$",
            re.I,
        ),
        "free -h",
    ),
    (
        re.compile(r"^(what\s+command\s+shows?|how\s+do\s+i\s+show)\s+(git\s+)?remotes?$", re.I),
        "git remote -v",
    ),
    (
        re.compile(
            r"^(what\s+command\s+shows?|how\s+do\s+i\s+show)\s+(git\s+)?staged\s+changes$", re.I
        ),
        "git diff --cached",
    ),
    (
        re.compile(
            r"^(what\s+command\s+checks?|how\s+do\s+i\s+check)\s+(the\s+)?python\s+version$", re.I
        ),
        "python3 --version",
    ),
    (
        re.compile(
            r"^(what\s+command\s+checks?|how\s+do\s+i\s+check)\s+(the\s+)?node\s+version$", re.I
        ),
        "node --version",
    ),
    (
        re.compile(
            r"^(what\s+command\s+checks?|how\s+do\s+i\s+check)\s+(the\s+)?go\s+version$", re.I
        ),
        "go version",
    ),
    (
        re.compile(
            r"^(what\s+command\s+checks?|how\s+do\s+i\s+check)\s+(the\s+)?rust\s+version$", re.I
        ),
        "rustc --version",
    ),
    (
        re.compile(
            r"^(what\s+command\s+shows?|how\s+do\s+i\s+show)\s+(the\s+)?directory\s+size$", re.I
        ),
        "du -sh .",
    ),
)
